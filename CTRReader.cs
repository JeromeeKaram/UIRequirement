...
class CTRReader
{
    ...
    public bool write(string outputDir, List<DamageInfo> lstDamages, string esn, string tsn, string csn, string finding)
    {
        try
        {
            //Update ESN/TSN
    ...
    private void read_PDF(string pdffile)
    {
        ...
        try
        {
            var oreader = new PDFReader();
            oreader.ReadPDF(pdffile);
            List<PDFPageInfo> lstPdfPages = oreader.m_lstPdfs;

            foreach (var opage in lstPdfPages)
            {
                int DescStart = -1;
                for (int i = 0; i < opage.m_lstHeights.Count; i++)
                {
                    var oht = opage.m_lstHeights[i];
